...
import { Store, Clock, MapPin, CheckCircle2, AlertCircle, Phone } from "lucide-react";
...
export default function PublicOrderForm() {
  ...
  // Flat list of selected items for submission
    ...
    Object.entries(quantities).forEach(([id, qty]) => {
      if (qty > 0) {
        ...
        for (const group of form.items) {
          const itemDef = group.items.find(i => i.id === id);
          if (itemDef) {
            list.push({
              itemId: id,
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      await submitMutation.mutateAsync({
        formId: form.id,
        data: { customerName, phone, address, items: selectedItemsList, totalAmount, totalItems },
      });
      setIsSubmitted(true);
      window.scrollTo(0, 0);
    } catch {
  ...
  if (isSubmitted) {
    // Group selected items by group info for the success screen
    const groupedSelected: { groupName: string; pickupTime: string; items: OrderItem[] }[] = [];
    for (const orderItem of selectedItemsList) {
      // find which group it belongs to
      for (const group of form.items) {
        const found = group.items.find(i => i.id === orderItem.itemId);
        if (found) {
          const existing = groupedSelected.find(g => g.groupName === group.groupName && g.pickupTime === group.pickupTime);
          if (existing) {
            existing.items.push(orderItem);
          } else {
            groupedSelected.push({ groupName: group.groupName, pickupTime: group.pickupTime, items: [orderItem] });
          }
          break;
        }
      }
      ...
      <div className="min-h-screen bg-[#F0F4F8] flex items-center justify-center p-4">
        <div className="bg-card p-8 rounded-3xl shadow-xl shadow-black/5 max-w-lg w-full text-center animate-in fade-in zoom-in duration-500">
          <div className="flex items-center justify-center gap-2 mb-2">
            ...
            <h2 className="text-lg font-bold text-foreground">Order Received!</h2>
          </div>
          ...
          <div className="bg-muted/50 rounded-2xl p-4 mb-5 text-left space-y-1">
            ...
            <div className="flex items-start text-sm">
              <MapPin className="w-3 h-3 text-primary mr-2 mt-0.5 shrink-0" />
              <p className="text-xs text-muted-foreground"><span className="font-semibold text-foreground">Address: </span>Shop No.3, 1621/J2, 16th Main Road, Anna Nagar West, Chennai 600040.</p>
            </div>
            <div className="flex items-start text-sm">
          ...
          <div className="bg-muted/50 rounded-2xl p-4 mb-6 text-left space-y-4">
            <h4 className="font-semibold text-foreground border-b border-border pb-2">Order Summary</h4>
            {groupedSelected.map((group, gi) => (
              <div key={gi}>
                <div className="flex items-center gap-1.5 mb-2">
                  <span className="font-semibold text-sm text-foreground">{group.groupName}</span>
                  <span className="text-xs text-muted-foreground bg-muted px-2 py-0.5 rounded-full flex items-center gap-1">
                    <Clock className="w-2.5 h-2.5" /> {group.pickupTime}
                  </span>
                </div>
    ...
    <div className="min-h-screen bg-[#F0F4F8] py-8 px-4 sm:px-6 font-sans">
      <div className="max-w-2xl mx-auto space-y-6 pb-24">
        ...
        <div className="bg-card rounded-2xl shadow-md shadow-black/5 overflow-hidden border border-border/50">
          ...
          <div className="p-4 pt-3">
            <h1 className="text-xl font-bold mb-2 leading-snug">{form.title}</h1>
            {form.description && (
      ...
      <div className="fixed bottom-0 left-0 w-full bg-card border-t border-border shadow-[0_-10px_40px_rgba(0,0,0,0.05)] z-50">
        <div className="max-w-2xl mx-auto px-4 py-3 flex items-center justify-between">
          <div>
            <div className="text-xs text-muted-foreground mb-0.5">Total ({totalItems} items)</div>
            <div className="text-lg font-semibold text-foreground">{formatCurrency(totalAmount)}</div>
          </div>
          <Button
